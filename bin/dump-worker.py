    ...
    def add_to_queue(self, data):
        try:
            ...
            if tweet.has_key('delete'):
                ...
                log.notice(u"Queued archived tweet {2} for user {0}/{1}".format(dict_mget(tweet, ['user', 'screen_name']), dict_mget(tweet, ['user', 'id_str']), tweet['id']))

            else:
                log.notice(u"Queued archived tweet: {0}".format(tweet))
...
class UserTimeLineClient(object):
    def __init__(self):
        self.config = tweetsclient.Config().get()
        consumer_key = self.config.get('tweets-get-client', 'consumer_key')
        consumer_secret = self.config.get('tweets-get-client', 'consumer_secret')
        access_token = self.config.get('tweets-get-client', 'access_token')
        access_token_secret = self.config.get('tweets-get-client', 'access_token_secret')
    ...
    def stream_forever(self):
        ...
        track_items = self.track.get_accounts()
        conn = self.track._get_database()
        log.debug(str(track_items))

        stream = None
        if stream_type == 'users':
            tweet_listener = UserTimeLine(self.beanstalk)
            api = tweepy.API(self.twitter_auth)
            tweetsclient.fetch(api, tweet_listener, track_items, conn)
